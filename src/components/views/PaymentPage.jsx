...
const PaymentPage = () => {
  ...
  useEffect(() => {
    const fetchLinks = async () => {
      setIsLoading(true);
      try {
        // Use the user endpoint for project links
        const response = await ApiService(`/user-projects/${projectId}/links`, 'GET');
        ...
        setLinks({ figma_url: '', payment_url: '' });
        // Optionally show a toast, but do not block rendering
        toast({
          title: 'Information',
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-bizzwiz-deep-space via-bizzwiz-deep-space/95 to-bizzwiz-deep-space text-bizzwiz-star-white flex">
      <Sidebar />
